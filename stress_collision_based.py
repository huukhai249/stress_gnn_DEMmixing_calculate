...
def process_dem_file(dem_path: Path, settings: StressSettings) -> None:
    print("-------------------------------------------------------")
    print(f"Loading collision-based analysis: {dem_path.name}")
    print("-------------------------------------------------------")
    ...
    print("-------------------------------------------------------")
    print(f"Processing collision-based metrics: {dem_path.name}")
    print("-------------------------------------------------------")

    data_csv_collision: list[dict[str, Any]] = []
    data_extract_collision: list[dict[str, Any]] = []
    host_rotational_distance_total_by_id: dict[int, float] = {}
    host_contact_time_total_by_id: dict[int, float] = {}
    ...
    for timestep_index in range(start_tstep, end_tstep + 1):
        ...
        if (
            ...
            print(
                f"Collision timestep {timestep_index} / {end_tstep} "
                f"(time={timestep_values[timestep_index]:.6f} s, step {processed_count + 1}/{total_timesteps})"
            )
        summary, _particle_extracted, collision_extracted = analyze_timestep(
            deck,
            timestep_index,
            timestep_values,
            settings,
        ...
        data_csv_collision.append({
            ...
            "Ave_TotalColForce_inSys_byCollision": summary["Ave_TotalColForce_inSys_byCollision"],
            "Ave_Fcol_Max_per_collision": summary["Ave_Fcol_Max_per_collision"],
            "Max_Fcol_Max_per_collision": summary["Max_Fcol_Max_per_collision"],
            "high_impact_event_fraction": summary["high_impact_event_fraction"],
            "Ave_SI_normal_per_collision": summary["Ave_SI_normal_per_collision"],
            "Ave_SI_shear_per_collision": summary["Ave_SI_shear_per_collision"],
            "Ave_SI_total_per_collision": summary["Ave_SI_total_per_collision"],
            "Total_normal_specific_energy_by_collision": summary["Total_normal_specific_energy_by_collision"],
            "Total_shear_specific_energy_by_collision": summary["Total_shear_specific_energy_by_collision"],
            "Total_specific_energy_by_collision": summary["Total_specific_energy_by_collision"],
            ...
            "Ave_normal_stress_value_per_collision": summary["Ave_normal_stress_value_per_collision"],
            "Ave_shear_stress_value_per_collision": summary["Ave_shear_stress_value_per_collision"],
            "Ave_stress_value_per_collision": summary["Ave_stress_value_per_collision"],
            "Ave_normal_power_per_collision": summary["Ave_normal_power_per_collision"],
            "Ave_shear_power_per_collision": summary["Ave_shear_power_per_collision"],
            "Ave_power_per_collision": summary["Ave_power_per_collision"],
            "Total_normal_power_by_collision": summary["Total_normal_power_by_collision"],
            "Total_shear_power_by_collision": summary["Total_shear_power_by_collision"],
            "Total_power_by_collision": summary["Total_power_by_collision"],
            ...
            "Ave_cumulative_contact_time_per_host_particle": summary["Ave_cumulative_contact_time_per_host_particle"],
        })

        if timestep_index == record_tstep and collision_extracted is not None:
            data_extract_collision.extend(expand_row_to_table(collision_extracted))

    extract_time_text = format_time_for_filename(settings.extract_data_time)

    if data_extract_collision:
        write_csv(dem_path.with_name(f"{dem_path.stem}_collision_extracted_at_{extract_time_text}.csv"), data_extract_collision)

    if data_csv_collision:
        output_file_summary_collision = dem_path.with_name(f"{dem_path.stem}_collision_over_time.csv")
        write_csv(output_file_summary_collision, data_csv_collision)
        print(f"Collision-based data saved to {output_file_summary_collision}")
